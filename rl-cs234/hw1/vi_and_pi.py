...
def policy_evaluation(P, nS, nA, policy, gamma=0.9, tol=1e-3):
	...
	while True:
		...
		for s in range(nS):
			...
			value_function[s] = sum([prob*(reward+(gamma*value_function[new_state])) for (prob, new_state, reward, _) in P[s][a]])
			threshold = max(threshold, abs(v - value_function[s]))
		if threshold < tol:
			break
...
def policy_improvement(P, nS, nA, value_from_policy, policy, gamma=0.9):
	...
	for s in range(nS):
		...
		best_action = None
		for a in range(nA):
			value_a = sum([prob*(reward+(gamma*value_from_policy[new_state])) for (prob, new_state, reward, _) in P[s][a]])
			if best_value < value_a:
				best_value = value_a
				best_action = a
...
def policy_iteration(P, nS, nA, gamma=0.9, tol=10e-3):
	...
	while policy_stable:
		...
		if (policy == new_policy).sum() == policy.shape[0]:
			policy_stable = False
		else:
			policy = new_policy
...
def value_iteration(P, nS, nA, gamma=0.9, tol=1e-3):
	...
	while True:
		...
		for s in range(nS):
			...
			best_value = -np.inf
			for a in range(nA):
				value_a = sum([prob*(reward+(gamma*value_function[new_state])) for (prob, new_state, reward, _) in P[s][a]])
				if best_value < value_a:
					best_value = value_a
